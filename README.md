...
```bash
npx web3sdkio create --template marketplace
```

- Create your own Marketplace contract via the web3sdkio dashboard. (Follow the steps in the guide below if you need extra help)!
...
- Head to the [web3sdkio dashboard](https://web3sdk.io/dashboard).
- Click **Create a new contract**.
- Click **Setup Marketplace**.
- Configure & Deploy!

## The Web3sdkio Provider

The web3sdkio React provider makes it straightforward to let your users connect their wallets to your website, and it abstracts away all the boilerplate you would usually have to write.

Open `pages/_app.tsx` we wrap all of our pages in the `<Web3sdkioProvider>` component.

```tsx
<Web3sdkioProvider desiredChainId={ChainId.Goerli}>
  <Component {...pageProps} />
</Web3sdkioProvider>
```
...
We connect user's wallets to our website by using the web3sdkio React SDK's [useMetamask](https://docs.web3sdk.io/react/react.usemetamask) hook.
...
Then, we use the [useContract](https://docs.web3sdk.io/react/react.useContract) hook to connect to our smart contract via it's contract address.
...
If you don't have NFTs that you can list, [you can create an NFT Collection via our dashboard](https://web3sdk.io/dashboard).
